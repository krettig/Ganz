...
class GanzSchonClever:
    ...
    def play_game(self):
        ...
        score, bad_things_summary = self.calculate_score(), self.strategy.end_game_summary()
        # Check if green is overplayed
        if bad_things_summary['overplayed_colors']['green'] > 0:
            # Output to the debug console
            if self.DEBUG_LEVEL > 2:
                logging.debug(f"Seed: {self.seed_value}, Score: {score}, Overplayed/Underplayed: {bad_things_summary}")

        return score, bad_things_summary
...
def print_summary(start_time, end_time, elapsed_time, i, average_score, debug_level, lowest_score, lowest_score_seed, highest_score, highest_score_seed):
    ...
    print(f"End Time: {end_time.strftime('%Y-%m-%d %H:%M:%S')}, Elapsed Time: {elapsed_time}, Games Played: {i + 1}, Average Score: {average_score:.0f}, debug_level: {debug_level}")
...
def update_minmix_scores(score, seed_value, lowest_score, lowest_score_seed, highest_score, highest_score_seed, debug_level):
    ...
    return lowest_score, lowest_score_seed, highest_score, highest_score_seed
...
# Factory Function
def create_strategy(manipulate_strategy=False):
    if manipulate_strategy:
        manipulated_data = execute_manipulate_strategy()
    else:
        manipulated_data = None
    return LoadStrategy(data=manipulated_data)

# Function to play the game num_games times and calculate average score
def play_games(num_games, seed=None, manipulate_strategy=False, strategy=None):
    os.system('cls' if os.name == 'nt' else 'clear')
    # Add a small delay to allow the terminal to reset its scroll position
    time.sleep(0.1)
    
    if strategy is None:
        strategy = create_strategy(manipulate_strategy)

    total_score = 0
    total_no_legal_plays = 0
    total_overplayed_colors = {color: 0 for color in ['yellow', 'blue', 'green', 'orange', 'purple']}
    total_underplayed_colors = {color: 0 for color in ['yellow', 'blue', 'green', 'orange', 'purple']}
    ...
    debug_level = 5 if num_games <= 2 else (2 if num_games <= 10 else 1)
    ...
    for i in range(num_games):
        ...
        score, bad_things = game.play_game()
        total_score += score
        total_no_legal_plays += bad_things['no_legal_plays']
        for color in total_overplayed_colors:
            total_overplayed_colors[color] += bad_things['overplayed_colors'][color]
            total_underplayed_colors[color] += bad_things['underplayed_colors'][color]

        lowest_score, lowest_score_seed, highest_score, highest_score_seed = update_minmix_scores(score, game.seed_value, lowest_score, lowest_score_seed, highest_score, highest_score_seed, debug_level)
    ...
    # Calculate averages
    average_no_legal_plays = total_no_legal_plays / num_games
    average_overplayed_colors = {color: total / num_games for color, total in total_overplayed_colors.items()}
    average_underplayed_colors = {color: total / num_games for color, total in total_underplayed_colors.items()}
    ...
    # Print results
    print(f"Average no legal plays (per 100 games): {round(average_no_legal_plays * 100)}")
    
    average_overplayed_colors_per_100 = {color: round(value * 100) for color, value in average_overplayed_colors.items()}
    print(f"Average overplayed colors (per 100 games): {average_overplayed_colors_per_100}")

    average_underplayed_colors_per_100 = {color: round(value * 100) for color, value in average_underplayed_colors.items()}
    print(f"Average underplayed colors (per 100 games): {average_underplayed_colors_per_100}")

    return average_score #if i + 1 > 9000 else 0

    

def test_strategy_parameter(parameter_name, start, end, increment, parameter_index=None, num_games=1000):
    original_strategy = LoadStrategy()
    original_data = original_strategy.data  # Assuming your strategy class has a 'data' attribute

    # Initialize variables to track the highest score and corresponding value
    highest_average_score = 0
    highest_score_value = None

    for value in np.arange(start, end + increment, increment):
        #print(f"Testing {parameter} with value: {value}")
        modified_data = deepcopy(original_data)
        if parameter_index is not None:
            modified_data[parameter_name][parameter_index] = value
        else:
            modified_data[parameter_name] = value
        strategy = LoadStrategy(data=modified_data)
        average_score = play_games(num_games, strategy=strategy)
        strategy.save_strategy_with_score(average_score)
        print(f"Average score for {parameter_name} = {value}: {int(average_score)}")

        # Update highest score and value if current score is higher
        if average_score > highest_average_score:
...
if run_forever:
    while True:
        ...
        if average_score > highest_average_score:
            ...
            strategy = LoadStrategy()
            strategy.save_strategy_with_score(average_score)
elif test_specific_parameter:
    # Define the parameters as local variables
    parameter = 'color_scores'
    #parameter_index = 0
    parameter_index = 'green'
    start_value = 0
    end_value = 100
    increment = 10
    num_games = 1000
    ...
    # Send the email
    ganz_utils.send_email(email_body=email_body)
